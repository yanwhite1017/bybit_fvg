...
blank = {'set_of_klines': [None, None], 'orders': {'type': None}, 'open_position': False}
set_of = {'symbols': {'ETHUSDT': {'set_of_klines': [None, None], 'orders': {'type': None}, 'open_position': False}}}

for pair in symbols:
	set_of['symbols'][pair] = blank

print(f'config_symbols: {set_of}')
		...
		if new_signal['fvg'] == True:
			...
			if new_signal['type'] == 'long':
				...
				if set_of['symbols'][name_pair]['open_position'] == False:
					stop_loss = toFixed(stop_loss, 2)
					take_profit = toFixed(take_profit, 2)
